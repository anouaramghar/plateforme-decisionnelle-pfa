...
**copilot-runtime** (Node/Express + CopilotKit) is an **optional** AI-assistant sidecar — never a startup gate, so the BI platform stays up if it's down. It calls an external LLM (NVIDIA NIM, `z-ai/glm-5.2` — a tool-calling model is required; the 70b llamas queue 60-90s on the shared free endpoint, and deepseek/nemotron break the agent loop). **Privacy note:** student data surfaced by Copilot tools (names, grades, risk levels) leaves the cluster to NVIDIA — treat this as a deliberate data-protection decision before using real student records. Copilot tools never run raw NL→SQL: `query_dw` maps keywords to fixed parameterized queries on a read-only DW login.
